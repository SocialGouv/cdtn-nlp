...
app = FastAPI(
    title="CDTN NLP",
    version="1.0",
    description="Code du travail numérique : information retrieval using neural networks",
)
...
@app.post("/embed", response_model=RecordsResponse, tags=["EMBED"])
async def embed(body: RecordsRequest = Body(..., example=example_request)):
    return {"embeddings":[]}


@app.post("/rerank", response_model=RecordsResponse, tags=["RANK"])
async def rank(body: RecordsRequest = Body(..., example=example_request)):
    return {"results":[]}
